...
public class TemplateFormValueDefaultServiceImp implements TemplateFormValueDefaultService {
    ...
    public void addDefaultValuesToForm(Long menuId, Long recordId, List<AddValueRequest> addValueRequests) {
        try {
            if(userServiceImp.getLoggedUser().getRole().equals(Role.ROLE_SUPERUSER)){
                if (recordId == null) {
                ...
                throw new CustomExceptions.CustomValidationException("User do not have permission to do this operation");
            }
        } catch (CustomExceptions.ResourceNotFoundException e) {
            throw e;
        } catch (Exception e) {
